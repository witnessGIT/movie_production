...
export interface SourceConfig { id: string; label: string; kind: 'rss'|'search_template'|'manual'; url: string; enabled: boolean; notes: string }
...
export interface Asset { id: string; project_id: string; kind: string; media_type: string; label: string; source: string; local_path?: string; size_bytes?: number; created_at: string; notes: string }
export interface Stage { id: string; label: string; state: StageState; progress: number; detail: string; artifact?: string }
export interface TimelineSegment {
  id: number; start: number; end: number; label: string; track: string
  asset_id?: string; media_type?: string; source_path?: string; source_start?: number; source_end?: number; score?: number
}
